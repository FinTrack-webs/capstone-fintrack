...
export default function RegisterPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await fintrackApi.register({ email, password });
      const loginResponse = await fintrackApi.login({ email, password });
      saveStoredAuth(loginResponse.data);
      router.push("/dashboard");
    } catch (error) {
      toast({
        title: "Gagal daftar",
        description: error instanceof Error ? error.message : "Coba gunakan email lain atau password minimal 6 karakter.",
        variant: "info",
      });
    } finally {
      setLoading(false);
    ...
    <main className="relative overflow-hidden grid min-h-dvh place-items-center bg-gradient-to-br from-[#f7f4ff] via-[#eefaf8] to-[#edf5ff] px-5 py-10 text-foreground dark:bg-[#101418]">
     ...
     <div className="absolute bottom-[-120px] right-[5%] h-[260px] w-[260px] rounded-full bg-[#d8c4ff]/25 blur-3xl" />
      <Card className="w-full max-w-md">
        ...
        <CardTitle className="mt-0 text-3xl">Daftar bentar aja</CardTitle>
        <CardText className="mt-4">Cukup email dan password saja. Simpel, sat-set.</CardText>
        <form className="mt-7 space-y-5" onSubmit={handleSubmit}>
          <div className="relative">
            <Mail className="absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-outline" />
            <Input className="pl-14" placeholder="Email aktif" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required />
          </div>
          <div className="relative">
            <LockKeyhole className="absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-outline" />
            <Input className="pl-14" placeholder="Password minimal 6 karakter" type="password" value={password} onChange={(event) => setPassword(event.target.value)} required minLength={6} />
          </div>
